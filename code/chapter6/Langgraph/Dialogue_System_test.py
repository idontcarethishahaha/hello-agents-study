...
llm = ChatOpenAI(
    ...
    # 新增的两段配置仅针对智谱做兼容，不锁死模型厂商：
    timeout=30,# timeout：通用超时参数，OpenAI 官方客户端也支持；
    # extra_body={}：用来给国产大模型传递自定义字段，切回原生 OpenAI 时删掉这行即可；
    model_kwargs={"extra_body": {}}  # 智谱兼容参数，消除OpenAI专属校验
)

# 初始化Tavily客户端（TAVILY_API_KEY变量名完全匹配你的env，无需改动）
...
def understand_query_node(state: SearchState) -> SearchState:
    ...
    for msg in reversed(state["messages"]):
        if isinstance(msg, HumanMessage):
            ...
            break
    if not user_message:
        return {"step": "error", "messages": [AIMessage(content="未读取到用户提问")]}
...
搜索词：[最佳搜索关键词]"""
    ...
    # 修复：必须同时传入System+Human消息，禁止只传System
    try:
        msg_list = [
            SystemMessage(content=understand_prompt),
            HumanMessage(content=f"用户原始提问：{user_message}")
        ]
        response = llm.invoke(msg_list)
    except Exception as e:
        print(f"理解节点LLM调用异常：{e}")
        return {"step": "error", "messages": [AIMessage(content=f"大模型调用失败：{str(e)}")]}
...
请提供一个有用的回答，并说明这是基于已有知识的回答。"""
        try:
            msg_list = [
                ...
                HumanMessage(content=f"用户原始提问：{user_origin_q}")
            ]
            response = llm.invoke(msg_list)
        except Exception as e:
            print(f"兜底回答LLM调用异常：{e}")
            return {"step": "completed", "messages": [AIMessage(content=f"模型调用失败：{str(e)}")]}
